  ...
  <div>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      <el-form class="small-space" :model="temp" :rules="rules" ref="temp" label-position="left" label-width="95px"
        ...
        <el-form-item label="收件员工号" prop="postUser">
          <el-input v-model="temp.postUser"></el-input>
        </el-form-item>
        ...
        <el-form-item label="收件员姓名" prop="operator">
          <el-input v-model="temp.operator"></el-input>
        </el-form-item>

        <el-form-item label="收件数量" prop="collectNumber">
          <el-input style="margin-top:8px;" v-model="temp.collectNumber"></el-input>
        </el-form-item>

        <el-form-item label="退件数量" prop="returnNumber">
          <el-input style="margin-top:8px;" v-model="temp.returnNumber"></el-input>
        </el-form-item>
  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          collectNumber: [
            {required: true, message: '请输入收件数量', trigger: 'blur'},
            { validator: uname, trigger: 'blur' }
          ],
          returnNumber: [
            {required: true, message: '请输入退件数量', trigger: 'blur'},
            { validator: uname, trigger: 'blur' }
          ],
    ...
    methods: {
      ...
      handleEdit(row){
        ...
        reportgetone(par).then(function (response) {
          console.log(JSON.parse(response.data))
          self.temp=JSON.parse(response.data).data
        })
